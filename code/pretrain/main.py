...
if __name__ == '__main__':
    ...
    # time_delta = [0,0,5,-5]
    # time_head = [10,10,10,10]
    time_delta = [9,10,15,4]
    time_head = [1,1,1,1]  
    head_number=20
    scheduler_time = 400
...
#     val_list=[0]
    ...
    if pretrain:
        ...
        model_best_dir=model_best_dir+'/pretrain'
        if not os.path.exists(model_best_dir):
            os.mkdir(model_best_dir)
    ...
    optimizer_mole = optim.Adam(
        [
            {'params': net_mole.parameters(),'lr':5*setting.lr_max,'momentum':0.9}
        ],
    )
    ...
    scheduler_mole = torch.optim.lr_scheduler.StepLR(optimizer_mole, scheduler_time, gamma=0.5)
    ...
    if pretrain:
        if  using_pretrain_model:
            ...
            temp_dir=model_best_dir
            front_net_crnn.load_state_dict(torch.load(temp_dir.replace('/pretrain','')+f'/front_net_crnn_ns{setting.ns}_nr{setting.nr}_best.pth',weights_only=True))
            back_net_crnn.load_state_dict(torch.load(temp_dir.replace('/pretrain','')+f'/back_net_crnn_ns{setting.ns}_nr{setting.nr}_best.pth',weights_only=True))
            net_mole.load_state_dict(torch.load(temp_dir.replace('/pretrain','')+f'/net_mole_ns{setting.ns}_nr{setting.nr}_best.pth',weights_only=True))
        ...
        optimizer = optim.Adam(
            [
                {'params': front_net_crnn.parameters(),'lr':5e-6,'momentum':0.9},
                {'params': back_net_crnn.parameters(),'lr':5e-6,'momentum':0.9},
                {'params': net_mole.parameters(),'lr':1e-3,'momentum':0.9}
            ],
        )
    ...
    for epoch in range(setting.n_epoch):
        ...
        if epoch >400:
            #修正back
            back_net_crnn.p_cutoff = 0.0005
            back_net_crnn.xiu2 = True
        ...
        for i,(ini_state,t,Y_label,X0,Temp,Pressure) in enumerate(front_dataloader):
            ...
            bvg = front_y_predict[-1][-1]
            solid = front_y_predict[-1][1]
            # loss4 = torch.relu(liblim-lib)*20+torch.relu(bvglim[case]-bvg)*20
            loss4 = torch.relu(liblim-lib)*20
            ...
            # if case == 0:
            #     loss6 = torch.relu(gas[front_end_idx:]-5).mean()*20+torch.relu(4.9-gas[front_end_idx:]).mean()*20
            # elif case == 1:
            #     loss6 = torch.relu(gas[front_end_idx:]-4.6).mean()*20+torch.relu(4.5-gas[front_end_idx:]).mean()*20

            loss6 = torch.relu(gas[front_end_idx:]-5).mean()*20

            # loss5 = (gas_v[front_end_idx]-Y_label[front_end_idx]).abs()*100

            loss5 = torch.relu(torch.sum(back_net_crnn.w_in_Ea)-Eamax*setting.nr_back)
...
###########################################################################
        ...
        with torch.no_grad():
            for i,(ini_state,t,Y_label,X0,Temp,Pressure) in enumerate(front_dataloader_test):
                ...
                grad = -(gas_v[1:] - gas_v[:-1])
                loss_grad = torch.sum(torch.clamp(grad,0,100))*1500   
                if  loss_grad>0:       
                    idex = torch.where(grad>0)[0][0]
                    #mole拉平
                    gas_vlast=gas_v[idex]
                    gas_vlast=torch.tensor([gas_vlast.item()]*(len(gas_v)-idex-1))
                    gas_v=torch.cat((gas_v[:idex+1], gas_vlast),dim=0)

                    # #gas拉平
                    # gasvlast=gas_v[idex]
                    # gaslast=gas[idex:]
                    # molelast=(gaslast/gasvlast).unsqueeze(1)
                    # mole=torch.cat((mole[:idex], molelast),dim=0)
