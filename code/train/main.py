...
#     val_list=[0]
    ...
    optimizer_mole = optim.Adam(
        [
            {'params': net_mole.parameters(),'lr':10*setting.lr_max,'momentum':0.9}
        ],
    )
    ...
    for epoch in range(setting.n_epoch):
        ...
        for i,(ini_state,t,Y_label,X0,Temp,Pressure) in enumerate(front_dataloader):
            ...
            loss6 = torch.relu(gas[front_end_idx:]-5.2).mean()*20

            mole_pre = 30 - mole[:int(len(t_front)*3/5)] 
            loss_mole = torch.relu(mole_pre).mean()*20
            ...
            loss=(loss1+loss2)+loss3+loss4+loss5+loss_mole+loss6
